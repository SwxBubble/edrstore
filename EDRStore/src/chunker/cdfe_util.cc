...
static inline uint64_t HashWindow(const uint8_t* data, int len) {
    uint64_t h = 0;
    for (int i = 0; i < len; ++i) {
...
uint64_t CDFEUtil::ExtractOneLocalFeature(const uint8_t* data, int size) const {
    ...
    if (size < feature_window_size_) {
        return Mix64(HashBytes(data, size));
    }

    const int window_count = size - feature_window_size_ + 1;
    uint64_t min_hash = UINT64_MAX;
    for (int off = 0; off < window_count; ++off) {
        const uint64_t h = Mix64(HashWindow(data + off, feature_window_size_));
        if (h < min_hash) {
            min_hash = h;
        }
    }

    return min_hash == UINT64_MAX ? Mix64(HashBytes(data, size)) : min_hash;
}
